...
export class AgentPulse {
  ...
  alert(title: string, severity: 'info' | 'warning' | 'critical' = 'warning', details?: string): void {
  ...
  async flush(): Promise<number> {
    ...
    try {
      ...
      if (this.debug) console.error(`[agentpulse] flush error:`, err);
  ...
  private enqueue(kind: string, data: Record<string, unknown>, session = ''): void {
    if (!this.enabled) return;
    if (this.queue.length >= QUEUE_MAX) {
      if (this.debug) console.warn('[agentpulse] queue full, dropping event');
      return;
    }
    this.queue.push({
      kind,
...
export function init(opts: InitOptions = {}): AgentPulse {
